...
public abstract class ServletInteractionContext implements WebInteractionContext {
  private static String COOKIE_USERNAME_NAME = "net.ugorji.oxygen.wiki.5.username";
  ...
  protected ServletInteractionContext(
    ...
    multipart =
        ("post".equals(request.getMethod().toLowerCase())
            && s != null
            && s.toLowerCase().startsWith(WebConstants.MULTIPART_PREFIX));
  }
  ...
  /**
   * Gets the username, searching in the following order - from cookie - from log-in username
   * (container managed) - return remote IP address
   */
  ...
  public void setUserName(String username) throws Exception {
    //     Cookie cookie = getCookie(req, COOKIE_USERNAME_NAME);
    //     if(cookie == null) {
    //       cookie = new Cookie(COOKIE_USERNAME_NAME, username);
    //     } else {
  ...
  public void removeUserName() throws Exception {
    //     Cookie cookie = getCookie(req, COOKIE_USERNAME_NAME);
    //     if(cookie != null) {
    //       //have to set all these, or it wouldn't work
    //       cookie.setPath("/");
    //       cookie.setMaxAge(0);
  ...
  private Cookie getCookie(String cookieName) {
    ...
    if (cookies != null && cookies.length > 0) {
      // WikiUtils.debug("Cookies: " + Arrays.asList(cookies));
      for (int i = 0; i < cookies.length; i++) {
        // WikiUtils.debug("Saw cookie: " + getCookieInfo(cookies[i]));
        if (cookieName.equals(cookies[i].getName())) {
          cookie = cookies[i];
          // WikiUtils.debug("Found cookie: " + getCookieInfo(cookies[i]));
          break;
        }
      }
    }
  ...
  public String getBaseURL() {
    // String s = null;
    // WebContainerEngine oce = WebLocal.getWebContainerEngine();
    // if(oce != null) {
    //  s = oce.getProperty(WebConstants.BASE_URL_KEY);
    // }
